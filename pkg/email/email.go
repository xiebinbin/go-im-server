// Package email 发送邮件
// 	email.Send(email.NewHTMLSender("这里是标题", email.ParseHHTML(client.FilePath() + "/bucket/email/test.html", struct {
//		Name string
...
func StartEmailSender() {
	once.Do(func() {
		_ = app.Config().Bind("client", "email", &conf)
		email = &Email{
...
func NewHTMLSender(subject string, html *HTML, to ...string) *Object {
	...
	object.writeHeader("SocketData-Type", "text/html;chartset=UTF-8")
	return object
}
