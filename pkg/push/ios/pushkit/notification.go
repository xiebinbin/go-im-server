...
// EPushType defines the value for the apns-notify-type header
type EPushType string

const (
	// PushTypeAlert is used for notifications that trigger a user interaction —
	// for example, an alert, badge, or sound. If you set this notify type, the
	// apns-topic header field must use your app’s bundle ID as the topic. The
	// alert notify type is required on watchOS 6 and later. It is recommended on
	...
	// background, and don’t trigger any user interactions. If you set this notify
	// type, the apns-topic header field must use your app’s bundle ID as the
	// topic. The background notify type is required on watchOS 6 and later. It is
	...
	// incoming Voice-over-IP (VoIP) call. If you set this notify type, the
	...
	// also register the certificate for VoIP services. The voip notify type is
	...
	// information for a watchOS app’s complications. If you set this notify type,
	...
	// services. The complication notify type is recommended for watchOS and iOS.
	...
	// extension. If you set this notify type, the apns-topic header field must
	// use your app’s bundle ID with .pushkit.fileprovider appended to the end.
	// The fileprovider notify type is not available on watchOS. It is recommended
	...
	// contact the MDM server. If you set this notify type, you must use the topic
	// from the UID attribute in the subject of your MDM notify certificate.
	PushTypeMDM EPushType = "mdm"
)

const (
	// PriorityLow will tell APNs to send the notify message at a time that takes
	// into account power considerations for the device. Notifications with this
	// priority might be grouped and delivered in bursts. They are throttled,
	// and in some cases are not delivered.
	PriorityLow = 5

	// PriorityHigh will tell APNs to send the notify message immediately.
	// Notifications with this priority must trigger an alert, sound, or badge
	// on the target device. It is an error to use this priority for a notify
...
type Notification struct {
	...
	// A byte array containing the JSON-encoded payload of this notify notification.
	...
	// The pushtype of the notify notification. If this values is left as the
	// default an apns-notify-type header with value 'alert' will be added to the
	// http request.
	PushType EPushType
}
