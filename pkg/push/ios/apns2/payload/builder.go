...
// InterruptionLevel sets the aps interruption-level on the payload.
//	{"aps":{"interruption-level":b}}
func (p *Payload) InterruptionLevel(i string) *Payload {
	p.aps().InterruptionLevel = i
	return p
}
...
// Notification SocketData app extension when a new notification arrives. If a
...
// SoundCritical sets the critical value on the aps critical dictionary.
// {"aps":{"sound":{"critical":1,"name":name,"volume":1.0}}}
func (p *Payload) SoundCritical(critical int) *Payload {
	p.aps().sound().Critical = critical
	return p
}
