...
func ApplyList(ctx context.Context, uid string, request IdsRequest) ([]members.ApplyRes, error) {
	dao := members.New()
	groupMembers, _ := dao.GetListsByStatusAndRole(uid, request.Ids, []int8{members.StatusIng, members.StatusRefuse}, []uint8{members.RoleOwner, members.RoleAdministrator})
	ids := make([]string, 0)
	memMap := make(map[string]members.ApplyRes)
	if len(groupMembers) > 0 {
		for _, v := range groupMembers {
	...
	res := make([]members.ApplyRes, 0)
	items, err := detail.New().GetInfoByIds(ids)
	fmt.Println("items: ", ids, items, err)
	if err != nil {
		return nil, err
	}
	if len(items) == 0 {
